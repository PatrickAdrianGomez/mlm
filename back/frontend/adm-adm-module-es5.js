      ...
      _createClass(DescendenciaFichaComponent, [{
        ...
        value: function ngOnInit() {
          ...
          this.connexion.get_dataWithParamsOne('personAssociated', '?userID=' + localStorage.getItem('code').substr(1) + '&companyName=' + localStorage.getItem('actual')).subscribe(function (regis) {
            _this27.person = regis[0];
            _this27.asociados = regis[0].associated;
      ...
      _createClass(DescendenciaListComponent, [{
        ...
        value: function obtenerDatos(state, id) {
          ...
          this.cnx.get_dataWithParams('personAssociated', '?userID=' + id + '&companyName=' + localStorage.getItem('actual') + '&estado=' + state).subscribe(function (myAds) {
            console.log(myAds);
            _this28.rows = myAds[0].associated;
            setTimeout(function () {
              _this28.isLoading = false;
            }, 1500);
            _this28.temp = myAds[0].associated.map(function (prop, key) {
        ...
        value: function onTreeAction(event) {
          ...
          if (row.treeStatus === 'collapsed') {
            ...
            this.cnx.get_dataWithParams('personAssociated', '?userID=' + row._id + '&companyName=' + myCompanyName[0].companyName + '&estado=' + row.estado).subscribe(function (myTreeAssoc) {
              ...
              _this29.temp = [].concat(_toConsumableArray(_this29.temp), _toConsumableArray(data));
            });
      ...
      _createClass(InvitationListComponent, [{
        ...
        value: function obtenerDatos(state) {
          ...
          this.cnx.get_dataWithParams('invitation', '?estado=' + state + '&ciMain=' + localStorage.getItem('code').substr(1) + '&companyName=' + localStorage.getItem('actual')).subscribe(function (myAds) {
            ...
            _this32.temp = myAds.map(function (prop, key) {
              return Object.assign({}, prop, {
                id: key
              });
            });
      ...
      _createClass(InvitationComponent, [{
        ...
        value: function verificarUsuario() {
          ...
          if (this.invitation.profile.ci) {
            this.connexion.get_dataWithParamsOne('personInvitation', '?ci=' + this.invitation.profile.ci + '&equipo=' + localStorage.getItem('actual')).subscribe(function (myUser) {
              if (myUser['id'] == 0) {
                _this35.usuarioValido = false;

                _this35.error(myUser['message'], 'Invitación a MLM');
              } else {
                _this35.usuarioValido = true;

                _this35.success(myUser['message'], 'Invitación a MLM');
              }
        ...
        value: function onRegister() {
          ...
          this.connexion.addObject('invitation', this.invitation).subscribe(function (regis) {
            ...
            _this36.correct = true;
            alert('La invitación fue realizada de manera correcta');

            _this36._router.navigate(['/']);
          }, function (error) {
            _this36.isSaved = false;
            var errorMessage = error;
            _this36.correct = false;

            switch (errorMessage.status) {
              case 400:
                _this36.usuarioValido = false;
                alert('El usuario ya fue invitado. \n Por favor intente nuevamente.');
                break;

              default:
                _this36.usuarioValido = true;
        ...
        value: function updateEmp(id) {
          ...
          this.ListCompany.forEach(function (comp) {
            if (id == comp._id) {
              ...
              _this37.equipoActual = comp.name;
            }
          });
        }
      }, {
    ...
    function () {
      ...
      _createClass(ProfileComponent, [{
        ...
        value: function ngOnInit() {
          ...
          if (codigo) {
            setTimeout(function () {
              _this39.connexion.get_dataId('person', codigo).subscribe(function (myUser) {
                console.log(myUser);
                _this39.perfil = myUser;
                _this39.urlImage = myUser.profile.photo.toString();
                _this39.listDepa = _this39.ListLOC.filter(function (LOC) {
                  return LOC.owner == myUser.contact.address.country;
                });
                _this39.listCiudad = _this39.ListLOC.filter(function (LOC) {
                  return LOC.owner == myUser.contact.address.state.id;
                });
                _this39.listZona = _this39.ListLOC.filter(function (LOC) {
      ...
      _createClass(ToastComponent, [{
        ...
        value: function ngOnInit() {
          ...
          this.subscription = this.toastService.onAlert(this.id).subscribe(function (alert) {
            ...
            _this41.alerts.push(alert);

            _this41.message = alert.message;
            _this41.title = alert.title;
            _this41.tipoAlert = _this41.cssClass(alert);
            $('.toast').toast('show');
          });
        }
      }, {
      ...
      _createClass(UploadComponent, [{
        ...
        value: function onUpload() {
          ...
          var _loop2 = function _loop2(index) {
            ...
            var nuevoNombre = _this42.changeName(_this42.selectedFile[index].name);

            try {
              fdScale.append('file_name', nuevoNombre);
              fdScale.append('file_path', _this42.path_server);
              fdScale.append('file', _this42.scaledImage[index], 'Scale' + dt.getUTCMinutes().toString() + dt.getUTCSeconds().toString() + dt.getUTCMilliseconds().toString() + nuevoNombre);

              _this42.http.post(src_app_services_globalVars__WEBPACK_IMPORTED_MODULE_4__["globalVars"].domain + '/api/v1/image/', fdScale, {
                reportProgress: true,
                observe: 'events',
                headers: head
              }).subscribe(function (event) {
                if (event.type === _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpEventType"].UploadProgress) {
                  _this42.porcentajeUpload = Math.round(event.loaded / event.total * 100) + "%";
                  document.getElementById("pbFile" + _this42.selectedFile[index].name).style.width = _this42.porcentajeUpload;
                  document.getElementById("pbFile" + _this42.selectedFile[index].name).innerHTML = _this42.porcentajeUpload;
                }
              });
            } catch (error) {
              console.log('Error: ' + error);
            ...
            try {
              ...
              this.http.post(src_app_services_globalVars__WEBPACK_IMPORTED_MODULE_4__["globalVars"].domain + '/api/v1/image/', fdThumb, {
                ...
                if (event.type === _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpEventType"].Response) {
                  _this42.desdeElHijo.emit({
                    upload: event.body
                  });

                  _this42.muestraMIDDLE = 'noMuestra';
                  _this42.muestraDOWN = 'noMuestra';
                }
              });
            } catch (error) {
              console.log('Error: ' + error);
      ...
      function ToastService(router) {
        ...
        this.router.events.subscribe(function (event) {
          if (event instanceof _angular_router__WEBPACK_IMPORTED_MODULE_2__["NavigationStart"]) {
            if (_this43.keepAfterRouteChange) {
              // only keep for a single route change
              _this43.keepAfterRouteChange = false;
            } else {
